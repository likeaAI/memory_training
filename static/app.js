...
const views = {
  ...
  viewPrepare: document.getElementById('viewPrepare'),
  viewMemorize: document.getElementById('viewMemorize'),
  viewTest: document.getElementById('viewTest'),
  viewResult: document.getElementById('viewResult'),
  viewSpatial: document.getElementById('viewSpatial'),
  viewSudoku: document.getElementById('viewSudoku')
};
...
async function triggerAutoSave(results, correctCount, total, accuracy) {
  try {
    ...
    // 좌측 사이드바 DB 최신 데이터로 실시간 갱신
    await loadSavedList();
  } catch (e) {
    console.error('DB 자동 저장 오류:', e);
  }
}
...
async function fetchAiCoaching(results, accuracy) {
  ...
  try {
    ...
    document.getElementById('coachSummaryText').innerText = data.analysis_summary || '분석 완료';
    document.getElementById('coachWeakText').innerText = data.weak_points || '없음 (양호)';
    document.getElementById('coachTipText').innerText = data.actionable_tip || '다음에도 강렬한 시각 이미지를 유지하세요.';
  } catch (err) {
    document.getElementById('coachSummaryText').innerText = '코칭 분석을 불러오지 못했습니다: ' + err;
  }
}

// SQLite 대시보드 및 저장된 목록 로드 (사용자별 격리)
async function loadSavedList() {
  ...
  try {
    ...
    const trainings = dbData.trainings || {};

    // 좌측 사이드바 SQLite 통계 & 세션 리스트 갱신
...
async function saveSpatialResult(isSuccess, clearedCount) {
  const isNoSave = document.getElementById('spatialNoSaveCheck')?.checked;
  if (isNoSave) {
    console.log('연습 모드(DB에 저장 안함)가 활성화되어 있어 결과를 저장하지 않습니다.');
    return;
  }
  ...
  try {
    ...
    console.error('공간기억 결과 저장 실패:', e);
  }
}

// 공간기억 SQLite 통계 대시보드 로드
...
async function loadMainDashboard() {
  try {
    ...
    // 1. 개념 훈련 DB & 공간 기억 DB & 스도쿠 DB 병렬 조회
    const [conceptRes, spatialRes, sudokuRes] = await Promise.all([
      fetch(`/api/dashboard?user_id=${userId}`).then(r => r.json()).catch(() => ({ sessions: [], trainings: {} })),
      fetch(`/api/spatial-dashboard?user_id=${userId}`).then(r => r.json()).catch(() => ({ history: [], max_span: 0, avg_rt: 0, total_wins: 0, total_plays: 0 })),
      fetch(`/api/sudoku-dashboard?user_id=${userId}`).then(r => r.json()).catch(() => ({ history: [], best_time: 0, avg_time: 0, total_clears: 0 }))
    ]);
